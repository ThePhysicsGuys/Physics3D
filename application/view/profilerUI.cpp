...
void BarChart::render() {
	...
	GUI::defaultFont->render(title, position + Vec2f(0, this->dimension.y - titleHeight), GUI::COLOR::WHITE, 0.001);
	...
	for (int cl = 0; cl < data.height; cl++) {
		GUI::defaultFont->render(classes[cl].name, drawingPosition + Vec2f(this->dimension.x - 0.3, drawingSize.y - 0.035 * cl), classes[cl].color, 0.0007);
	}
}
