...
void SpiderFactory::buildSpider(const GlobalCFrame& spiderPosition) {
	...
	spiderBody->material.albedo = Color(0.6f, 0.6f, 0.6f, 1.0f);
...
void buildTree(Position treePos) {
	...
	trunk->material.albedo = Graphics::COLOR::get(0x654321);
	...
	for (int j = 0; j < 15; j++) {
		...
		leaves->material.albedo = Vec4(fRand(-0.2, 0.2), 0.6 + fRand(-0.2, 0.2), 0.0, 1.0);
...
void buildTerrain(double width, double depth) {
	...
	for (double x = -width / 2; x < width / 2; x += 3.0) {
		for (double z = -depth / 2; z < depth / 2; z += 3.0) {
			...
			newPart->material.albedo = Color(0.0, yOffset / 40.0 + 0.5, 0.0, 1.0);
...
void buildCar(const GlobalCFrame& location) {
	...
	carWindshield->material.albedo = Color(0.7f, 0.7f, 1.0f, 0.5f);
