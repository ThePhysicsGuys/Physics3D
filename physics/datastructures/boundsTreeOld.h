...
struct BoundsTree {
	...
	void addToGroup(Boundable* obj, const Bounds& bounds, TreeNode& groupNode) {
	...
	void addToGroup(Boundable* obj, const Bounds& bounds, const Boundable* objInGroup, const Bounds& objInGroupBounds) {
		NodeStack iter = findGroupFor(objInGroup, objInGroupBounds);
		addToGroup(obj, bounds, **iter);
		iter.expandBoundsAllTheWayToTop();
	}

	void addToGroup(Boundable* obj, const Boundable* objInGroup) {
		this->addToGroup(obj, obj->getBounds(), objInGroup, objInGroup->getBounds());
	}

	void addToGroup(TreeNode&& newNode, const Boundable* objInGroup, const Bounds& objInGroupBounds) {
	...
	void addAllToGroup(BoundableIterBegin begin, BoundableIterEnd end, Boundable* objInGroup, const Bounds& objInGroupBounds) {
		NodeStack iter = findGroupFor(objInGroup, objInGroupBounds);
		for(; begin != end; ++begin) {
			Boundable* obj = *begin;
			addToGroup(obj, obj->getBounds(), **iter);
		}
		iter.expandBoundsAllTheWayToTop();
	}
	template<typename BoundableIterBegin, typename BoundableIterEnd>
	void addAllToGroup(BoundableIterBegin begin, BoundableIterEnd end, Boundable* objInGroup) {
		addAllToGroup(begin, end, objInGroup, objInGroup->getBounds());
	}

	// merges the groupNode of second into the groupNode of first
	void mergeGroups(Boundable* first, const Bounds& firstBounds, Boundable* second, const Bounds& secondBounds) {
	...
	void mergeGroups(Boundable* first, Boundable* second) {
		this->mergeGroups(first, first->getBounds(), second, second->getBounds());
	}
	...
	template<typename BoundableIterBegin, typename BoundableIterEnd>
	void splitGroup(BoundableIterBegin begin, BoundableIterEnd end) {
		assert(begin != end);
	...
	inline void forEach(const Func& func) const {
		for(const Boundable& p : *this) {
			func(p);
		}
	}
	template<typename Func>
	inline void forEach(const Func& func) {
		for(Boundable& p : *this) {
			func(p);
		}
	}
