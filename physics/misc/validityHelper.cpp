...
static void recursiveTreeValidCheck(const OldBoundsTree::TreeNode& node, bool hasAlreadyPassedGroupHead) {
	...
	if(node.isLeafNode()) {
		...
		for(OldBoundsTree::TreeNode& n : node) {
			recursiveTreeValidCheck(n, node.isGroupHead || hasAlreadyPassedGroupHead);
		}
	}
}

static void recursiveCheckTreeBounds(const OldBoundsTree::TreeNode& node) {
	if(!node.isLeafNode()) {
...
static int countOccurences(const void* obj, const OldBoundsTree::TreeNode& node) {
	if(node.isLeafNode()) {
...
static void recursiveCheckNoDuplicates(const OldBoundsTree::TreeNode& node, std::set<const void*>& foundObjects, std::set<const OldBoundsTree::TreeNode*>& foundNodes) {
	...
	if(node.isLeafNode()) {
		...
		for(const OldBoundsTree::TreeNode& subNode : node) {
...
void treeValidCheck(const OldBoundsTree::TreeNode& rootNode) {
	...
	std::set<const OldBoundsTree::TreeNode*> foundNodes;
